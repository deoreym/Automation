...
public class FooterCustomizerTest extends BaseClass {
	...
	//Check when privacy policy is enabled from customizer and redirecting to the expexted url entered
	@Test(priority=1)
	public void verifyprivacypolicywhenenabled() throws InterruptedException {
		...
		if(status==true) {
			...
			softAssert.assertEquals(actualurl, a_ppl," ON clicking privacy link it is not redirecting to expected url ");
			driver.get(homepage);
		}
		else {
			if(status==false) {
	...
	//Check when terms and condition is enabled from customizer and redirecting to the expexted url entered
	@Test(priority=3)
	public void verifyverifytermsandconditionwhenenabled() throws InterruptedException {
		...
		if(status==true) {
			...
			String actualurl=driver.getCurrentUrl();
			softAssert.assertEquals(actualurl, a_ppl," ON clicking terms and condition  link it is not redirecting to expected url ");
			driver.get(homepage);
		}
		else {
			if(status==false) {
				...
				fcp.f_termsandcond.click();
				String actualurl=driver.getCurrentUrl();
				softAssert.assertEquals(actualurl, a_ppl," ON clicking terms and condition it is not redirecting to expected url ");
			}
			driver.get(homepage);	
		}
		softAssert.assertAll();
	...
	public void verifycopyrightswhenenabled() throws InterruptedException {
		...
		if(status==true) {
			...
			softAssert.assertEquals(copyright_text, a_copyrights," The copyright text is not getting matched with expected text ");
			driver.get(homepage);
		}
		else {
			if(status==false) {
	...
	//Check when poweredby is enabled from customizer and redirecting to the expexted url entered
	...
	public void verifypoweredbywhenenabled() throws InterruptedException {
		...
		if(fcp.f_poweredby.size()==1) {
				...
				Assert.assertEquals(actualTitle, expected_remuititle,"on clicking edwiser remui link it is not redirecting to the expected page");
				}
		else {
			dp.remuimenu();
			cpp.customizertab.click();
